...
theme_set(theme_bw())
d$Rrel <- d$R_tau/d$R0
g <- ggplot(d[d$R02 %in% c("R0 = 1.5")&d$i0==1e-04&d$c2%in% c("c = 0.05")&d$tFinal %in% c(60),], aes(x = tau,y = Rrel,linetype=as.factor(optimization)))+geom_line()+facet_grid(R02~c2,space="free")+theme(legend.position = "bottom")+xlab(TeX(r'($\tau$)'))+ylab(TeX(r'($R_D/R_0$)'))
g
```
```{r}
theme_set(theme_bw())
...
gm1 <- ggplot(d[d$R02 %in% c("R0 = 1.5")&d$i0==1e-04&d$c2%in% c("c = 0.05")&d$tFinal %in% c(60),], aes(x = tau,y = Rrel,linetype=as.factor(optimization)))+geom_line()+facet_grid(R02~c2,space="free")+theme(legend.position = "bottom")+xlab(TeX(r'($\tau$)'))+ylab(TeX(r'($R_D/R_0$)'))+labs(linetype="Optimization\ntype")+guides(linetype=FALSE)
gm1
```
...
gm3 <- ggplot(d[d$R02 %in% c("R0 = 1.5")&d$i0==1e-04&d$c2%in% c("c = 0.05")&d$tFinal %in% c(60),], aes(x = tau,y = i,linetype=as.factor(optimization)))+geom_line()+facet_grid(R02~c2,space="free")+theme(legend.position = "bottom")+xlab(TeX(r'($\tau$)'))+ylab(TeX(r'($i$)'))+labs(linetype="Optimization\ntype")
...
```
<style>
.slides > section > .columns-3 > .column {
  width: 33.33%;
}
</style>
```{r}
