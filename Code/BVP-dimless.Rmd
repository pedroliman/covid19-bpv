...
sol.explore <- NULL
t.vec <- c(4,8,16,32,64)
for (tFinal in t.vec){
    t <- seq(0,tFinal,1/7)
    capture.output(sol <- bvpshoot(yini = yini,yend=yend,x=t,parms = list(R_0 = R_0,c=c),func = oneComparmentSIR,maxiter = 200))
    
res <- try(sol <- bvptwp(yini = yini,yend=yend,x=t,parms = list(R_0 = R_0,c=c),func = oneComparmentSIR,xguess = t,yguess = t(sol[,2:5])))
  ...
  if(inherits(res, "try-error")){
    ...
    res <- try(sol <- bvptwp(yini = yini,yend=yend,x=t,parms = list(R_0 = R_0,c=c),func = oneComparmentSIR,xguess = t,yguess = t(sol.prev[,2:5])))
  }}
