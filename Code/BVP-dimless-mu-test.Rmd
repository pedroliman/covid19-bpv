  ...
  unique.costs <- c()
    ...
    res <- try(bvptwp(yini = yini,yend=yend,x=t,parms = list(R_0=R_0),func = oneComparmentSIRmu,
      ...
      if(solved==TRUE){
        ...
        neighbor.rows <- which(exper.design[,c("neighbor_1", "neighbor_2","neighbor_3","neighbor_4")]==case.id,arr.ind=TRUE)[,1]
        neighbor.cols <- which(exper.design[,c("neighbor_1", "neighbor_2","neighbor_3","neighbor_4")]==case.id,arr.ind=TRUE)[,2]
        neighbor.cols <- paste0("solved_", neighbor.cols)
        for (n in neighbor.cols){
        exper.design[neighbor.rows,n] <- TRUE
        }
        if(solved.count == 1){
          sol.explore2 <- sol
        } else {
...
write.csv(sol.explore2,"INTERMEDIATE/mu.solutions0.csv")
```
