...
with(as.list(c(y, parms)),{
  ...
  return(list(c(ds1,ds2,di1,di2,dmu1,dmu2,dnu1,dnu2)))
})
}
...
print(Sys.time())
  ...
  unique.costs <- c()
    ...
    solved <- FALSE
     ...
     xguess <- guess[,"tau"]
     yguess <- t(guess[,c("s1","s2","i1","i2","mu1","mu2","nu1","nu2")])
     ...
     # print(gc())
      ...
      if(solved==TRUE){
        ...
        write.table(
          sol,
          file = paste0("INTERMEDIATE/mu.solutions.2groups.csv"),
