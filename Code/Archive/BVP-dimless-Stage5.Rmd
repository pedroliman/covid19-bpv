...
```{r}
sol.explore <- read.csv("INTERMEDIATE/sol.explore-step4.csv")
exper.design <- read.csv("INTERMEDIATE/exper.design-step4.csv")
```
...
print(paste("c*tFinal is",tFinal*c))
  same.tFinal <- exper.design.s[exper.design.s$tFinal == tFinal,]
 # dists <- dist.matrix[same.tFinal$case.id,case.run]
#  cases <- names(sort(dists)[1:5])
#  same.tFinal <- subset(exper.design.s,case.id %in% cases)
    solved <- FALSE
    for(case.id.c in same.tFinal$case.id){
     if(solved == FALSE){
      guess <- subset(sol.explore,case.id == case.id.c)
      s.init <- interp1(guess$tau,guess$s,xi=t,method="linear")
      i.init <- interp1(guess$tau,guess$i,xi=t,method="linear")
      lambda_s.init <- interp1(guess$tau,guess$lambda_s,xi=t,method="linear")
      lambda_i.init <- interp1(guess$tau,guess$lambda_i,xi=t,method="linear")
      guess <- as.data.frame(cbind(t,s.init,i.init,lambda_s.init,lambda_i.init))
      names(guess) <- c("t","s","i","lambda_s","lambda_i")
      if(length(t) < dim(guess)){
        guess <- guess[1:length(t),]
      } else if(length(t) > dim(guess)){
        guess[(dim(guess)[1]+1):length(t),]<- guess[dim(guess)[1],]
      }
      worked <- TRUE
if(c<=same.tFinal[which(same.tFinal$case.id==case.id.c),"c"]&!(same.tFinal[which(same.tFinal$case.id==case.id.c),"c"] %in% c.tried)){
    # Here we try the two-point method. Note we silenced the error so if it doesn't converge
    # we use the shooting method solution. 
     c.end <- c
     c.start <- same.tFinal[which(same.tFinal$case.id==case.id.c),"c"]
     print(paste("c start is",c.start))
     ratio <- (c.end/c.start)^(.1)
     c.vec <- c.start*ratio^(seq(0,10,1))
  print(paste("c vec is",c.vec))
        for(c in c.vec){
          print(paste("c is",c))
          if(worked){
    res <- try(guess <- bvptwp(yini = yini,yend=yend,x=t.temp,parms = list(R_0 = R_0,c=c),
    ...
    exper.design[case.run,"twp.success5"] <- !inherits(res,"try-error")
   ...
   solved <- TRUE
    ...
    print(solved)}
print(Sys.time())
write.csv(sol.explore,"INTERMEDIATE/sol.explore-step5.csv",row.names = FALSE)
write.csv(exper.design,"INTERMEDIATE/exper.design-step5.csv",row.names = FALSE)
