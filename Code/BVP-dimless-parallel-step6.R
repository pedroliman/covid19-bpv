...
exper.design.u <- subset(exper.design,R0 == 6 & case.id>417)
print(Sys.time())
        c.tried <- c()
 ...
 same.c <- exper.design.s[order(exper.design.s$dist),]
    ...
    for(case.id.c in same.c$case.id){
     if(solved == FALSE | solved.count < 10){
...
write.csv(sol.explore2,paste0("INTERMEDIATE/sol.explore.case-",case.id,".csv"),row.names = FALSE)
