...
```{r}
sol.explore <- read.csv("INTERMEDIATE/sol.explore-step9.csv")
exper.design <- read.csv("INTERMEDIATE/exper.design-step9.csv")
```
...
guess <- as.data.frame(guess)
    ...
    res <- try(guess <- bvptwp(yini = yini,yend=yend,x=t,parms = list(R_0 = R_0,c=c),
                         func = oneComparmentSIR,xguess = t,yguess = t(guess[,c("s","i","lambda_s","lambda_i")])),
               silent=FALSE)
        t.temp <- guess[,1]
    print(length(t.temp))
   ...
   solved <- TRUE
    ...
    print(solved)}
print(Sys.time())
write.csv(sol.explore,"INTERMEDIATE/sol.explore-step10.csv",row.names = FALSE)
write.csv(exper.design,"INTERMEDIATE/exper.design-step10.csv",row.names = FALSE)
