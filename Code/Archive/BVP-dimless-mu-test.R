...
sol.explore <- read.csv(paste0("INTERMEDIATE/mu.solutions.csv"),row.names = NULL)
    ...
    res <- try(bvptwp(yini = yini,yend=yend,x=t,parms = list(R_0=R_0),func = oneComparmentSIRmu,
     ...
     # print(gc())
      ...
      if(solved==TRUE){
        ...
        write.table(
          sol,
          file = paste0("INTERMEDIATE/mu.solutions",".csv"),
...
write.csv(neighbors,paste0("INTERMEDIATE/mu.neighbors",iter.num,".csv"),row.names=FALSE)
