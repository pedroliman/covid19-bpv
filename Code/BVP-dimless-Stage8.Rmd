...
```{r}
sol.explore <- read.csv("INTERMEDIATE/sol.explore-step7.csv")
exper.design <- read.csv("INTERMEDIATE/exper.design-step7.csv")
```

```{r}
exper.design.s <- exper.design[exper.design$twp.success==TRUE,]
exper.design.u <- exper.design[exper.design$twp.success==FALSE&exper.design$c %in% c(0.025,0.05,0.1,0.125,0.2,0.4),]
```
...
for (case.run in exper.design.u$case.id){
  ...
  if(dim(same.tFinal)[1]==0){
    same.tFinal <- exper.design.s[sample(1:dim(exper.design.s)[1],10),]
  }
   ...
   i0 <- exper.design$i0[case.run]
     print(paste("case id is the same as case run?",case.run==case.id))
   print(tFinal)
    t <- seq(0,tFinal,1/7)
    yini = c(1-i0,i0,NA,NA)
    yend = c(NA,NA,0,0)
    ...
    for(case.id.c in same.tFinal$case.id){
     if(solved != TRUE){
      ...
      worked <- TRUE
      if(R_0>=same.tFinal[which(same.tFinal$case.id==case.id.c),"R0"]){
    # Here we try the two-point method. Note we silenced the error so if it doesn't converge
    # we use the shooting method solution. 
        R_0.vec <- seq(same.tFinal[which(same.tFinal$case.id==case.id.c),"R0"],R_0,by=0.1)
        for(R_0 in R_0.vec){
          if(worked){
   ...
   solved <- TRUE
    ...
    print(solved)}
print(Sys.time())
write.csv(sol.explore,"INTERMEDIATE/sol.explore-step8.csv")
write.csv(exper.design,"INTERMEDIATE/exper.design-step8.csv")
