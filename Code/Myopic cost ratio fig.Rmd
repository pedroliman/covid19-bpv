...
g2b <-  merged %>%
 ...
 scale_fill_continuous_divergingx(palette = 'PuOr', mid = 0,name="Infection\nCost\nDifference\n(Myopic-KTH)")+
  xlab(TeX(r'($T_{final}$)'))+theme(legend.position = "bottom")+facet_grid(.~as.factor(R0))+ylab(TeX(r'($c$)'))+facet_nested(.~lab2+as.factor(R0))+theme(legend.position = "bottom")+ggtitle("(b) Infection Cost Difference")
g2b

```
```{r}
png("FIGS/Figure 2.png",width=3.5,height=8,units='in',res=600)
plot_grid(g2a,g2b,ncol=1,rel_heights = c(1,.8))
dev.off()
```
```{r}
...
```
```{r}
png("FIGS/Figure 3.png",width=3.5,height=8,units='in',res=600)
...
g4b <-  merged %>%
  ...
  geom_tile() + scale_fill_viridis_c(name="Cost\nDifference\n(Myopic-KTH)",option="A")+xlab(TeX(r'($T_{final}$)'))+theme(legend.position = "bottom")+facet_grid(.~as.factor(R0))+ylab(TeX(r'($c$)'))+facet_nested(.~lab2+as.factor(R0))+theme(legend.position = "bottom")+ggtitle("(b) Cost Difference")
g4b

```

```{r}
png("FIGS/Figure 4.png",width=3.5,height=8,units='in',res=600)
plot_grid(g4a,g4b,ncol=1,rel_heights = c(1,.8))
dev.off()
```
...
```{r,echo=FALSE}
png("FIGS/Figure 3.png",width=3.5,height=6,units='in',res=600)
plot_grid(g5,g5b,labels=c("(a)","(b)"),ncol=1)
dev.off()
