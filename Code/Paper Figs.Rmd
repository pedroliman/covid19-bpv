...
```
```{r}
g <- ggplot(data=d[d$i0==0.0001&d$R0==1.5&d$tFinal==36,],aes(x=tau,y=R_tau,color=c,linetype=is.min,group=as.factor(case.id.c)))+geom_point()+scale_color_viridis()
g

```

```{r}
g <- ggplot(data=d[d$i0==0.0001&d$R0==1.5&d$c>.24&d$c<.31&d$tFinal==40,],aes(x=tau,y=R_tau,group=as.factor(case.id.c)))+geom_point(size=0.1)+scale_color_viridis()+facet_wrap(.~as.factor(c))+ylab("RD")
g

```
```{r,echo=FALSE}
png(filename = "FIGS/local_min.png",height = 3,width=4,units = "in",res = 800)
g
dev.off()
...
```{r}
g <- ggplot(data=d[d$i0==0.0001&d$R0==1.5&d$c>.24&d$c<.31&d$tFinal==40,],aes(x=tau,y=R_tau,color=finalEpi))+geom_point(size=0.1)+ylab("RD")+scale_color_viridis()
g

```
```{r,echo=FALSE}
png(filename = "FIGS/local_min_together.png",height = 3,width=4,units = "in",res = 800)
g
dev.off()
...
```{r}
g <- d %>% 
  filter(tFinal %in% c(40)&i0==0.0001&c%in% c(0.24,0.25,0.26,0.27,0.28,0.29,0.3,0.31)&R0==1.5) %>%
