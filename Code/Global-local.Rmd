...
tFinal.vec <- seq(8,80,by=4)
g <-  d %>%
  filter(tau == tFinal & i0==0.0001&tFinal %in% tFinal.vec&R0<6) %>%
  mutate(finalEpi = 1-s, is.min2=factor(is.min,levels=c(TRUE,FALSE))) %>%
  ggplot(mapping= aes(x = tFinal, y = c,fill=finalEpi)) +
  geom_tile() + scale_fill_viridis_c(name="Final\nEpidemic\nSize")+xlab(TeX(r'($T_{final}$)'))+theme(legend.position = "bottom")+facet_grid(is.min2~as.factor(R0))
g
...
g <- d %>% 
  ...
  ggplot(mapping=aes(x=tau,y=s,color=as.factor(c))) + geom_line()+facet_grid(.~is.min)+  scale_color_manual(values = c("#1b9e77", "#d95f02")) 
...
g <- d %>% 
  filter(tFinal %in% c(20)&i0==0.0001&c>0.07&c<.1&R0==4.5) %>%
  ggplot(mapping=aes(x=tau,y=RD,color=as.factor(c))) + geom_point()+facet_grid(.~is.min)
g
...
g <- d %>% 
  filter(tFinal %in% c(20)&i0==0.0001&c>0.07&c<.1&R0==4.5) %>%
  ggplot(mapping=aes(x=tau,y=i,color=as.factor(c))) + geom_line()+facet_grid(.~is.min)
g
...
```{r}
g <- d %>% 
  filter(tFinal %in% c(20)&tFinal==tau&i0==0.0001&R0==4.5) %>%
  ggplot(mapping=aes(x=c,y=Cost,color=as.factor(is.min))) + geom_point()
g
```
```{r}
mult.sol <- d %>% 
